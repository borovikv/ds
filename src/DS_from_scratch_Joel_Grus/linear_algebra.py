...
def dot(v, w):
    """
    Скаля́рное произведе́ние (иногда внутреннее произведение) — операция над двумя векторами, результатом которой является число
    (когда рассматриваются векторы, числа часто называют скалярами),
